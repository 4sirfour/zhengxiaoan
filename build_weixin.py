...
function answer(q){
  ...
  if(q.includes("代收") && !/不能代收/.test(parts.join("")))
    parts.push(`<div class="hl">⚠️ 口径提示：若委托书涉及授权受托人<b>代收房款</b>——按知识库口径，<b>卖房款项不能代收</b>（条目 1「要求」）。请以办理公证处最终口径为准。</div>`);
  // 计费口径：股权类
  if(/股权/.test(q) && !/标的费/.test(parts.join("")))
    parts.push(`<div class="hl">💰 计费口径：股权类公证按<b>标的费+公证费</b>计收——标的费约按标的额 1.2%（实际以公证处核算为准），公证费另计。需实质审查股权真实性；公司有港资的，另需香港持证律师对营业执照和公司章程审查并出具证明。</div>`);
  // 受理口径：合同/协议公证
  if(/(合同公证|协议公证)/.test(q) && !/受理红线|均可办理/.test(parts.join("")))
    parts.push(`<div class="hl">📌 受理口径：双方签署的各类合同/协议均可办理合同公证或协议公证；<b>合同内容违法或违背公序良俗的不予受理</b>。涉及财产标的的按标的费+公证费计费。</div>`);
  return parts.join("\\n");
}
